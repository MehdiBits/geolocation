...
from utils.feature_extraction import extract_clip_features_from_image, rotate_image_randomly_symmetry
...
for idx, img_name in enumerate(img_dir):
    ...
    image, rotation_angle = rotate_image_randomly_symmetry(image, 30)
    features = extract_clip_features_from_image(image)
    ...
    results.append({'image': img_name, 'matched_image': best_img_id, 'similarity': similarity_score, 'angle': rotation_angle})

# Save results
pd.DataFrame(results).to_csv(f'results/predictions_{os.path.basename(image_dir_path)}_rotated.csv', index=False)

generate_geolocation_csv(results, PRECOMPUTED_METADATA_PATH, 'ressources/image_coordinates_im2_gps2k.csv', f'results/geolocation_results_{os.path.basename(image_dir_path)}_rotated.csv')
